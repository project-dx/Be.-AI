  ...
  return (
    <div className="space-y-5">
      {showUserCta && !data.today_report.exists && (
        <Link
          to="/daily-report"
          className="block rounded-2xl bg-emerald-600 px-5 py-4 text-white hover:bg-emerald-700"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-white border border-slate-200 p-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-500">ストレス状態</p>
            {stress ? (
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-white border border-slate-200 p-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold text-slate-500">目標達成率</p>
            {data.goal_achievement_rate == null ? (
              <p className="text-sm text-slate-400">目標なし</p>
            ) : (
              <p className="text-2xl font-bold text-sky-700">{data.goal_achievement_rate}<span className="text-xs font-normal text-slate-500">%</span></p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* 今日の提案 */}
      <Card title="🌟 今日の提案" accent="text-emerald-700">
        {recommendations.length === 0 ? (
          <EmptyState message="AI分析が実行されると、今日できる小さな提案が表示されます" />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {recommendations.slice(0, 3).map((rec, i) => (
              <div key={i} className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-4">
        ...
        <Card title="📋 支援計画の進捗">
          {data.support_plan ? (
            <div className="space-y-2">
              <p className="font-bold text-slate-700 text-sm">{data.support_plan.title}</p>
              ...
              {data.support_plan.next_review_date && (
                <p className="text-xs text-slate-500">次回見直し日: {data.support_plan.next_review_date}</p>
              )}
            </div>
          ) : (
